...
gbm_clf = GradientBoostingClassifier(max_depth=5, learning_rate=0.1, n_estimators=200, min_samples_split=3, subsample=0.8, random_state=16)

lrl1_clf = LogisticRegression(penalty='l1', C=0.5, solver="liblinear", random_state=20)
lrl2_clf = LogisticRegression(penalty='l2', C=0.5, solver="liblinear")
lgbm_clf = lightgbm.LGBMClassifier(objective='binary', max_depth=5, num_leaves=30, learning_rate=0.1, n_estimators=1000, min_child_samples=80,
...
df = pd.read_csv(r'C:\Users\69559\Desktop\out56\69positive.csv')

certain_feature_names = ['original_glrlm_LowGrayLevelRunEmphasis',
...
trian, test = train_test_split(df, test_size=0.3, random_state=15)
...
# 创建SVM分类器
# model = svm.SVC()
model = VotingClassifier(estimators=[('rf', rnd_clf), ('svc', svm_clf), ('knn', knn_clf), ('ada', ada_clf), ('gbm', gbm_clf), ('lgbm', lgbm_clf), ('lr1', lrl1_clf),('cat',cat_clf)],
...
for train_index, test_index in kfold.split(train_X, train_Y):
    ...
    for clf in (rnd_clf, svm_clf, knn_clf, gbm_clf, ada_clf, lgbm_clf,  lrl1_clf, cat_clf, model):
