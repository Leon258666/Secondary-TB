...
rnd_clf = RandomForestClassifier(n_estimators=200, min_samples_leaf=3, min_samples_split=2, oob_score=True, class_weight='balanced', random_state=42)
svm_clf = SVC(probability=True,random_state=16)
knn_clf = KNeighborsClassifier(n_neighbors=8, p=1, weights='distance')
ada_clf = AdaBoostClassifier(n_estimators=20, base_estimator=svm_clf, learning_rate=1, random_state=42)
lda_clf = LinearDiscriminantAnalysis()
gbm_clf = GradientBoostingClassifier(random_state=16)
...
lgbm_clf = lightgbm.LGBMClassifier(objective='binary', max_depth=4, num_leaves=30, learning_rate=0.05, n_estimators=1000, min_child_samples=80,
...
df = pd.read_csv('C:/Users/Lenovo/Desktop/out56/80positive.csv')

certain_feature_names = ['gradient_firstorder_Kurtosis',
...
trian, test = train_test_split(df, test_size=0.3, random_state=20)
...
# 创建SVM分类器
# model = svm.SVC()
model = VotingClassifier(estimators=[('rf', rnd_clf), ('svc', svm_clf), ('knn', knn_clf), ('ada', ada_clf), ('lda', lda_clf), ('gbm', gbm_clf), ('lr1', lrl1_clf), ('lr2', lrl2_clf), ('lgbm', lgbm_clf), ('cat', cat_clf)],#estimators:子分类器
...
for train_index, test_index in kfold.split(train_X, train_Y):
    ...
    for clf in (rnd_clf, svm_clf, knn_clf, ada_clf, lda_clf, gbm_clf, lrl1_clf, lrl2_clf, lgbm_clf, cat_clf, model):
