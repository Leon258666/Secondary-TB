...
svm_clf = SVC(C=2, decision_function_shape='ovr', probability=True, random_state=16)
...
gbm_clf = GradientBoostingClassifier(max_depth=3, learning_rate=0.1, n_estimators=200, min_samples_split=3, subsample=0.8, random_state=16)
...
df = pd.read_csv('C:/Users/Lenovo/Desktop/out56/91positive.csv')
...
'squareroot_gldm_DependenceEntropy',
...
# 创建SVM分类器
# model = svm.SVC()
model = VotingClassifier(estimators=[('rf', rnd_clf), ('svc', svm_clf), ('knn', knn_clf), ('ada', ada_clf), ('lda', lda_clf), ('gbm', gbm_clf), ('lr1', lrl1_clf), ('lr2', lrl2_clf)],
...
for train_index, test_index in kfold.split(train_X, train_Y):
    ...
    for clf in (rnd_clf, svm_clf, knn_clf, lda_clf, gbm_clf, ada_clf, lrl1_clf, lrl2_clf, model):
